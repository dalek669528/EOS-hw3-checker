...
void check_1(vector<log_file> log_files){
    ...
    for(int i=0 ; i<log_files.size() ; i++){
        ...
        else if(command.compare(0, 3, "buy") == 0){
                ...
                if( ((mask[0] + mask[1] + mask[2]) <= 3) && mask[0]>=0 && mask[1]>=0 && mask[2]>=0){
                    ...
                    if( mydata.stores_map.find(store) != mydata.stores_map.end() ){
                        ...
                        if( mydata.stores_map[store].cities_map.find(city_idx) != mydata.stores_map[store].cities_map.end() ){
                            ...
                            // else{
                            //     correct_answer(log_files[i].index,"Buy failed_1.", log_files[i].results);
                            // }
                        }
                            // else{
                            //     correct_answer(log_files[i].index,"Buy failed_2.", log_files[i].results);
                            // }
                    }
                            // else{
                            //     correct_answer(log_files[i].index,"Buy failed_3.", log_files[i].results);
                            // }
                }
                            // else{
                            //     correct_answer(log_files[i].index,"Buy failed_4.", log_files[i].results);
                            // }
            }
                            // else{
                            //     correct_answer(log_files[i].index,"Buy failed_5.", log_files[i].results);
                            // }
...
int main(int argc, char *argv[]){
    ...
    cout << "Use " << diff(time1, time2).tv_sec << "." << diff(time1, time2).tv_nsec <<" s to process "<<total_cmd<<" commands"<< endl;
